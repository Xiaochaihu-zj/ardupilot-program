...
const AP_Param::GroupInfo AP_MotorsHeli_Single::var_info[] PROGMEM = {
    ...
    // @Range: -10 10
    // @Increment: 0.1
    AP_GROUPINFO("COLYAW", 7,  AP_MotorsHeli_Single, _collective_yaw_effect, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("TAIL_SPEED", 9, AP_MotorsHeli_Single, _direct_drive_tailspeed, AP_MOTORS_HELI_SINGLE_DDVPT_SPEED_DEFAULT),
	
	
    // @Param: COL_DIFF_VALUE
    // @DisplayName: collective diffe value
    // @Description: collective differential value ,using value cos(radians(_col_diff))
    // @Range: 0 180
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("COL_DIFF_VALUE", 10, AP_MotorsHeli_Single, _col_diff, DEFAULT_COL_DIFF_ANGLE),
	

	// @Param: LON_DIFF_VALUE
    // @DisplayName: display
    // @Description: longitudinal differential value ,using value cos(radians(_long_diff))
    // @Range: 0 180
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("LON_DIFF_VALUE", 11, AP_MotorsHeli_Single, _long_diff, DEFAULT_LONG_DIFF_ANGLE),
	
	// @Param: LON_SYN_VALUE
    // @DisplayName: dispaly
    // @Description: longitudinal syn value ,using value cos(radians(_long_syn))
    // @Range: 0 180
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("LON_SYN_VALUE", 12, AP_MotorsHeli_Single, _long_syn, DEFAULT_LONG_SYN_ANGLE),
...
void AP_MotorsHeli_Single::set_update_rate( uint16_t speed_hz )
{
    ...
    uint32_t mask = 
		...
		1U << pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_9]) |
		1U << pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_10]) |
		1U << pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_11]);
...
void AP_MotorsHeli_Single::enable()
{
    ...
    hal.rcout->enable_ch(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_9]));
    hal.rcout->enable_ch(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_10]));
    hal.rcout->enable_ch(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_11]));	// yaw
...
#if 0
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_5]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_6]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_7]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_8]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_9]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_10]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_11]);
	RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_MOT_12]);
#endif
}
...
void AP_MotorsHeli_Single::init_outputs()
{
	...
	reset_swash_servo (_swash_servo_4);//eidt by huhui 
	reset_swash_servo (_swash_servo_5);
	reset_swash_servo (_swash_servo_6);
	
	_swash_servo_1.set_reverse(true);
	_swash_servo_2.set_reverse(true);
	_swash_servo_4.set_reverse(true);
	_swash_servo_5.set_reverse(true);
	
	
	_judge_col_diff=_col_diff;
	_judge_long_diff=_long_diff;
	_judge_long_syn=_long_syn;
...
void AP_MotorsHeli_Single::calculate_scalars()
{
    ...
    _roll_scaler = (float)_cyclic_max/4500.0f;//2500
    _pitch_scaler = (float)_cyclic_max/4500.0f;
    _collective_scalar = ((float)(_collective_max-_collective_min))/1000.0f;//0.5
...
void AP_MotorsHeli_Single::calculate_roll_pitch_collective_factors()
{
    if (_swash_type == AP_MOTORS_HELI_SINGLE_SWASH_CCPM) {                     //CCPM Swashplate, perform control mixing
        ...
        // roll factors		
		//cosf(radians(_servo1_pos + 90 - (_phase_angle + _delta_phase_angle)));//0
        _rollFactor[CH_1] = cosf(radians(_col_diff)); // edit by huhui 
        _rollFactor[CH_2] = cosf(radians(_col_diff));
        _rollFactor[CH_3] = cosf(radians(_col_diff));
		_rollFactor[CH_4] = -cosf(radians(_col_diff));
		_rollFactor[CH_5] = -cosf(radians(_col_diff));
		_rollFactor[CH_6] = -cosf(radians(_col_diff));

        // pitch factors
        _pitchFactor[CH_1] = cosf(radians(-90));//edit by huhui 
        _pitchFactor[CH_2] = cosf(radians(_long_syn+180));
        _pitchFactor[CH_3] = cosf(radians(_long_syn));
		_pitchFactor[CH_4] = cosf(radians(-90));
		_pitchFactor[CH_5] = cosf(radians(_long_syn+180));
		_pitchFactor[CH_6] = cosf(radians(_long_syn));

        // collective factors
        _collectiveFactor[CH_1] = 1;//edit by huhui 
		...
		_yawFactor[CH_1] = cosf(radians(-90));//edit by huhui 
        _yawFactor[CH_2] = cosf(radians(_long_diff));
        _yawFactor[CH_3] = cosf(radians(_long_diff+180));
		_yawFactor[CH_4] = cosf(radians(-90))/2.0f;
		_yawFactor[CH_5] = cosf(radians(_long_diff+180));
		_yawFactor[CH_6] = cosf(radians(_long_diff));
...
void AP_MotorsHeli_Single::move_actuators(int16_t roll_out, int16_t pitch_out, int16_t coll_in, int16_t yaw_out)
{
	...
	//edit by huhui
	if(_judge_col_diff!=_col_diff||_judge_long_diff!=_long_diff||_judge_long_syn!=_long_syn)
	{
		calculate_roll_pitch_collective_factors();
	    _judge_col_diff=_col_diff;
	    _judge_long_diff=_long_diff;
	...
	_tilt_servo.tilt_output();
	
    int16_t yaw_offset = 0;
    int16_t coll_out_scaled;
	int16_t yaw_output=0;//edit by huhui
	int16_t aileron_output=0;
	int16_t elevator_output=0;
	int16_t rudder_output=0;
	...
	aileron_output=1500+roll_out * _rpy_scalar;//edit by huhui 
	elevator_output=1500+pitch_out *_rpy_scalar;
	rudder_output=1500+yaw_out *_rpy_scalar;
    ...
    if (total_out > _cyclic_max) {
        float ratio = (float)_cyclic_max / total_out;//2500
        roll_out *= ratio;
        pitch_out *= ratio;
    ...
    coll_out_scaled = _collective_out * _collective_scalar + _collective_min - 1000;//0.5   1250
    ...
    if (_servo_mode == SERVO_CONTROL_MODE_AUTOMATED) {
        ...
        if ((_main_rotor.get_control_output() > _rsc_idle_output) && _tail_type != AP_MOTORS_HELI_SINGLE_TAILTYPE_SERVO_EXTGYRO) {//0   1
            // sanity check collective_yaw_effect
            _collective_yaw_effect = constrain_float(_collective_yaw_effect, -AP_MOTORS_HELI_SINGLE_COLYAW_RANGE, AP_MOTORS_HELI_SINGLE_COLYAW_RANGE);//0
	...
	yaw_output=yaw_out + yaw_offset;//edit by huhui 

    // swashplate servos  
    _swash_servo_1.servo_out = (_rollFactor[CH_1] * roll_out + _pitchFactor[CH_1] * pitch_out +_yawFactor[CH_1]*yaw_output)/10 + _collectiveFactor[CH_1] * coll_out_scaled + (_swash_servo_1.radio_trim-1500);
    _swash_servo_2.servo_out = (_rollFactor[CH_2] * roll_out + _pitchFactor[CH_2] * pitch_out +_yawFactor[CH_2]*yaw_output)/10 + _collectiveFactor[CH_2] * coll_out_scaled + (_swash_servo_2.radio_trim-1500);
	_swash_servo_3.servo_out = (_rollFactor[CH_3] * roll_out + _pitchFactor[CH_3] * pitch_out +_yawFactor[CH_3]*yaw_output)/10 + _collectiveFactor[CH_3] * coll_out_scaled + (_swash_servo_3.radio_trim-1500);
    _swash_servo_4.servo_out = (_rollFactor[CH_4] * roll_out + _pitchFactor[CH_4] * pitch_out +_yawFactor[CH_4]*yaw_output)/10 + _collectiveFactor[CH_4] * coll_out_scaled + (_swash_servo_4.radio_trim-1500);
    _swash_servo_5.servo_out = (_rollFactor[CH_5] * roll_out + _pitchFactor[CH_5] * pitch_out +_yawFactor[CH_5]*yaw_output)/10 + _collectiveFactor[CH_5] * coll_out_scaled + (_swash_servo_5.radio_trim-1500);
    _swash_servo_6.servo_out = (_rollFactor[CH_6] * roll_out + _pitchFactor[CH_6] * pitch_out +_yawFactor[CH_6]*yaw_output)/10 + _collectiveFactor[CH_6] * coll_out_scaled + (_swash_servo_6.radio_trim-1500);
#if 0  //edit by huhui 
   if (_swash_type == AP_MOTORS_HELI_SINGLE_SWASH_H1) {
	...
	_swash_servo_4.calc_pwm();//edit by huhui 
	...
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_4]), _swash_servo_4.radio_out);//edit by huhui 
	...
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_7]), aileron_output);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_8]), elevator_output);
	hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_9]), rudder_output);
 
    // update the yaw rate using the tail rotor/servo
    // move_yaw(yaw_out + yaw_offset);  //edit by huhui 
}
