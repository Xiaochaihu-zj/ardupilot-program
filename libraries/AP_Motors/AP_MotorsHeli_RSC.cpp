...
void AP_MotorsHeli_RSC::write_rsc(int16_t servo_out)
{
    if (_control_mode == ROTOR_CONTROL_MODE_DISABLED){
		...
		//_servo_output_channel

        hal.rcout->write(CH_10, _servo_output.radio_out);//edit by huhui 
    }
}

void AP_MotorsHeli_RSC::tilt_output()
{
	hal.rcout->write(_servo_output_channel, _tilt_servo_pos);
}