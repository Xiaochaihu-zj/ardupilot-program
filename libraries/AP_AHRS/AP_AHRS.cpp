...
void AP_AHRS::update_mti_atti_values(void)
{//edit by huhui 
    mti_roll_sensor  = degrees(mti_roll) * 100;
    mti_pitch_sensor = degrees(mti_pitch) * 100;
    mti_yaw_sensor   = degrees(mti_yaw) * 100;
    if (mti_yaw_sensor < 0)
        mti_yaw_sensor += 36000;
}


void AP_AHRS::set_mti_dcm(void)
{
	_mti_cos_roll=cosf(mti_roll);
	_mti_cos_pitch=cosf(mti_pitch);
	_mti_cos_yaw=cosf(mti_yaw);
	_mti_sin_roll=sinf(mti_roll);
	_mti_sin_pitch=sinf(mti_pitch);
	_mti_sin_yaw=sinf(mti_yaw);
}
