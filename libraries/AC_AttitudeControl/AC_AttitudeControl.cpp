...
void AC_AttitudeControl::relax_bf_rate_controller()
{
    ...
    const Vector3f& gyro = _ahrs.mti_gyro();
...
void AC_AttitudeControl::angle_ef_roll_pitch_rate_ef_yaw_smooth(float roll_angle_ef, float pitch_angle_ef, float yaw_rate_ef, float smoothing_gain)
{
    ...
    if ((_accel_roll_max > 0.0f) && _rate_bf_ff_enabled) {
        ...
        angle_ef_error.x = wrap_180_cd_float(_angle_ef_target.x - _ahrs.mti_roll_sensor);
    ...
    if ((_accel_pitch_max > 0.0f) && _rate_bf_ff_enabled) {
        ...
        angle_ef_error.y = wrap_180_cd_float(_angle_ef_target.y - _ahrs.mti_pitch_sensor);
...
void AC_AttitudeControl::angle_ef_roll_pitch_yaw(float roll_angle_ef, float pitch_angle_ef, float yaw_angle_ef, bool slew_yaw)
{
    ...
    angle_ef_error.x = wrap_180_cd_float(_angle_ef_target.x - _ahrs.mti_roll_sensor);
    angle_ef_error.y = wrap_180_cd_float(_angle_ef_target.y - _ahrs.mti_pitch_sensor);
    angle_ef_error.z = wrap_180_cd_float(_angle_ef_target.z - _ahrs.mti_yaw_sensor);
...
void AC_AttitudeControl::rate_bf_roll_pitch_yaw(float roll_rate_bf, float pitch_rate_bf, float yaw_rate_bf)
{
    ...
    // Update angle error
    if (labs(_ahrs.mti_pitch_sensor)<_acro_angle_switch) {
        ...
        if (frame_conversion_bf_to_ef(_angle_bf_error, angle_ef_error)) {
            _angle_ef_target.x = wrap_180_cd_float(angle_ef_error.x + _ahrs.mti_roll_sensor);
            _angle_ef_target.y = wrap_180_cd_float(angle_ef_error.y + _ahrs.mti_pitch_sensor);
            _angle_ef_target.z = wrap_360_cd_float(angle_ef_error.z + _ahrs.mti_yaw_sensor);
        }
...
void AC_AttitudeControl::frame_conversion_ef_to_bf(const Vector3f& ef_vector, Vector3f& bf_vector)
{
    ...
    bf_vector.x = ef_vector.x - _ahrs.mti_sin_pitch() * ef_vector.z;
    bf_vector.y = _ahrs.mti_cos_roll()  * ef_vector.y + _ahrs.mti_sin_roll() * _ahrs.mti_cos_pitch() * ef_vector.z;
    bf_vector.z = -_ahrs.mti_sin_roll() * ef_vector.y + _ahrs.mti_cos_pitch() * _ahrs.mti_cos_roll() * ef_vector.z;
}
...
void AC_AttitudeControl::update_ef_roll_angle_and_error(float roll_rate_ef, Vector3f &angle_ef_error, float overshoot_max)
{
    ...
    angle_ef_error.x = wrap_180_cd(_angle_ef_target.x - _ahrs.mti_roll_sensor);//edit by huhui 
    ...
    _angle_ef_target.x = angle_ef_error.x + _ahrs.mti_roll_sensor;
...
void AC_AttitudeControl::update_ef_pitch_angle_and_error(float pitch_rate_ef, Vector3f &angle_ef_error, float overshoot_max)
{
    ...
    angle_ef_error.y = wrap_180_cd(_angle_ef_target.y - _ahrs.mti_pitch_sensor);//edit by huhui
    ...
    _angle_ef_target.y = angle_ef_error.y + _ahrs.mti_pitch_sensor;
...
{
    // calculate angle error with maximum of +- max angle overshoot
    angle_ef_error.z = wrap_180_cd(_angle_ef_target.z - _ahrs.mti_yaw_sensor);//edit by huhui 
    ...
    _angle_ef_target.z = angle_ef_error.z + _ahrs.mti_yaw_sensor;//edit by huhui
...
void AC_AttitudeControl::integrate_bf_rate_error_to_angle_errors()
{
    ...
    _angle_bf_error.x += (_rate_bf_desired.x - (_ahrs.mti_gyro().x * AC_ATTITUDE_CONTROL_DEGX100)) * _dt;
    ...
    _angle_bf_error.y += (_rate_bf_desired.y - (_ahrs.mti_gyro().y * AC_ATTITUDE_CONTROL_DEGX100)) * _dt;
    ...
    _angle_bf_error.z += (_rate_bf_desired.z - (_ahrs.mti_gyro().z * AC_ATTITUDE_CONTROL_DEGX100)) * _dt;
...
void AC_AttitudeControl::update_rate_bf_targets()
{
	...
	_rate_bf_target.x += _angle_bf_error.y * _ahrs.mti_gyro().z;
	_rate_bf_target.y += -_angle_bf_error.x * _ahrs.mti_gyro().z;
}
...
float AC_AttitudeControl::rate_bf_to_motor_roll(float rate_target_cds)
{
    ...
    current_rate = (_ahrs.mti_gyro().x * AC_ATTITUDE_CONTROL_DEGX100);//edit by huhui 
...
float AC_AttitudeControl::rate_bf_to_motor_pitch(float rate_target_cds)
{
    ...
    // get current rate
    // To-Do: make getting gyro rates more efficient?
    current_rate = (_ahrs.mti_gyro().y * AC_ATTITUDE_CONTROL_DEGX100);
...
float AC_AttitudeControl::rate_bf_to_motor_yaw(float rate_target_cds)
{
    ...
    // get current rate
    // To-Do: make getting gyro rates more efficient?
    current_rate = (_ahrs.mti_gyro().z * AC_ATTITUDE_CONTROL_DEGX100);
