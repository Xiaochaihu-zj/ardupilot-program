...
void NavEKF::ResetVelocity(void)
{
    if (constPosMode || PV_AidingMode != AID_ABSOLUTE) {
        ...
        state.velocity.x  = velNED.x + gpsVelGlitchOffset.x; // north velocity from blended accel data    velNED come from gps
...
bool NavEKF::getPosNED(Vector3f &pos) //const
{
	...
	//edit by huhui
	mti_position=_ahrs->get_gps().location();
	Vector2f mti_pos_diff=location_diff(EKF_origin,mti_position);
	pos.x=mti_pos_diff.x;
	pos.y=mti_pos_diff.y;
	pos.z=EKF_origin.alt-mti_position.alt;
	return true;
#if 0	
...
bool NavEKF::getLLH(struct Location &loc) //const  
{
	...
	mti_position=_ahrs->get_gps().location();
	loc= mti_position;
	loc.alt = EKF_origin.alt - mti_altitude;
	return true;
#if 0
    if(validOrigin) {
...
void NavEKF::readGpsData()
{
            ...
            (_ahrs->get_gps().status() >= AP_GPS::GPS_OK_FIX_3D))
    {
        ...
        velNED = _ahrs->get_gps().velocity();//state.ground_speed * cosf(gps_heading)
...
//edit by huhui
void NavEKF::convert_frame_bf_to_ef(Vector3f &body_frame,Vector3f &earth_frame) 
{
	earth_frame.x = body_frame.x + _ahrs->sin_roll() * (_ahrs->sin_pitch()/_ahrs->cos_pitch()) * body_frame.y + _ahrs->cos_roll() * (_ahrs->sin_pitch()/_ahrs->cos_pitch()) * body_frame.z;
    earth_frame.y = _ahrs->cos_roll()  * body_frame.y - _ahrs->sin_roll() * body_frame.z;
    earth_frame.z = (_ahrs->sin_roll() / _ahrs->cos_pitch()) * body_frame.y + (_ahrs->cos_roll() / _ahrs->cos_pitch()) * body_frame.z;
    
}

void NavEKF::get_mti_accel_ef(Vector3f &mti_accel_ef) 
{
