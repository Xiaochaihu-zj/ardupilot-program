...
void Copter::check_dynamic_flight(void)
{
    ...
    if (position_ok()) {
        ...
        moving = (motors.get_throttle() > 800.0f || ahrs.mti_pitch_sensor < -1500);
    }
...
void  Copter::heli_update_tilt_servo_target()//edit by huhui 
{
	tilt_control = g.rc_7.radio_in>1500;
	if(tilt_control)
	{
		motors.set_tilt_servo_pos(motors.get_tilt_high_pos());
	}
	else
	{
		motors.set_tilt_servo_pos(motors.get_tilt_low_pos());
	}
}
