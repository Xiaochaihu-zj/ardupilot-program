...
#define ARM_DELAY              20//20  // called at 10hz so 2 seconds
#define DISARM_DELAY           20//20  // called at 10hz so 2 seconds
#define AUTO_TRIM_DELAY       100//100 // called at 10hz so 10 seconds
#define LOST_VEHICLE_DELAY     10//10  // called at 10hz so 1 second
...
bool Copter::init_arm_motors(bool arming_from_gcs)
{
    ...
    initial_armed_bearing = ahrs.mti_yaw_sensor;
...
bool Copter::arm_checks(bool display_failure, bool arming_from_gcs)
{
    ...
    // check lean angle
    if ((g.arming_check == ARMING_CHECK_ALL) || (g.arming_check & ARMING_CHECK_INS)) {
        if (degrees(acosf(ahrs.mti_cos_roll()*ahrs.mti_cos_pitch()))*100.0f > aparm.angle_max) {
            if (display_failure) {
