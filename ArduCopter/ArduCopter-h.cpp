...
const AP_Scheduler::Task Copter::scheduler_tasks[] PROGMEM = {
    ...
    { SCHED_TASK(read_mti_AHRS),          8,   75 },//edit by huhui 
	{ SCHED_TASK(set_ekf_origin),         40,   75},
...
void Copter::init_simple_bearing()
{
    ...
    super_simple_last_bearing = wrap_360_cd(ahrs.mti_yaw_sensor+18000);
    super_simple_cos_yaw = simple_cos_yaw;
    super_simple_sin_yaw = simple_sin_yaw;

    // log the simple bearing to dataflash
    if (should_log(MASK_LOG_ANY)) {
        Log_Write_Data(DATA_INIT_SIMPLE_BEARING, ahrs.mti_yaw_sensor);
    }
}
...
void Copter::read_mti_AHRS(void)
{
	
	while (mti_serial->available()) 
	{
		//mti_serial->printf(PSTR("read number=%d"),mti_serial->available());
		ch = mti_serial->read();//读取串口
		mtig_ReceiveDataByte(ch);
	};
}

void Copter::mtig_ReceiveDataByte(unsigned char recdata)
{
	if(read_number>=MTI_Length)
	  {r_state=0;
     read_number=0;}
	...
	switch(r_state)							  
	{
		...
		case 1:  
			if(recdata == 0xFF)
			{
				r_state = 2;
				sum += recdata;
				//mti_serial->printf(PSTR("receive packet\n"));
			}
			else 
			{
				if(recdata == 0xFA)
		...
		case 2:
			mtig_messageID = recdata;
			r_state = 3;
			sum += recdata;
			if(mtig_messageID!=0x36)
				r_state=0;
		break;	
		...
		case 3:
			...
			p_count = 0;
			sum += recdata;
			//mti_serial->printf(PSTR("received data length=%d\n"),MTI_Length)	;	
		break;	
			
		case 4:
		    read_number+=0x1;
			buff[p_count++] = recdata;
			if(p_count==2)
			{
				if(buff[0]==0x20&&buff[1]==0x30)
				{mti_id=ATTITUDE_ANGLE;
				r_state=5;
				p_count=0;}
				...
				r_state=5;
				p_count=0;}
				else if(buff[0]==0xD0&&buff[1]==0x10)
			    {mti_id=VELOCITY;
			    r_state=5;
				p_count=0;}
			    ...
			    r_state=5;
				p_count=0;}
				else if(buff[0]==0x50&&buff[1]==0x20)
				{mti_id=ALTITUDE;
			    r_state=5;
				p_count=0;}
				else if(buff[0]==0x50&&buff[1]==0x40)
				{mti_id=LNG_LAT;
			    r_state=5;
				p_count=0;}
				else {
		...
		case 5:
		    read_number+=0x1;
			MTI_data_length=recdata;
			r_state=6;
			//mti_serial->printf(PSTR("received data packet length=%d\n"),MTI_data_length);
		break;	
		
		case 6:
		    read_number+=0x1;
		    buff[p_count++] = recdata;
			if(p_count>=MTI_data_length)
			{
				mtig_MessageReceived(mti_id, buff, MTI_data_length);
				p_count=0;
				r_state=4;
			}
		break;
...
void Copter::mtig_MessageReceived(int id, unsigned char* mtig_data, unsigned char len)
{
	union  packet
{
	float MTI_a;
	unsigned char  MTI_b[4];
	double MTI_c;
}MTI_p;
	int n=0,m=0,data_length,mti_packet=0;//算出参数
	float	fchar[18];
	switch(id)
	{
        case ATTITUDE_ANGLE:		
	        data_length=len/4;
			for(int mti_j=0;mti_j<data_length;mti_j++)
			{
				for(int j=0;j<=3;j++)
				{
					MTI_p.MTI_b[3-j]=*(mtig_data+n);
					n++;
				}
				fchar[mti_packet++]=MTI_p.MTI_a;
			}
			MTI_ins.MTI_attitude.x=fchar[m++]*DEG_TO_RAD_MTI;
			MTI_ins.MTI_attitude.y=fchar[m++]*DEG_TO_RAD_MTI;
			MTI_ins.MTI_attitude.z=wrap_360_cd_yaw(450.0f-fchar[m++])*DEG_TO_RAD_MTI;
			
			ahrs.mti_roll = MTI_ins.MTI_attitude.x;
	        ahrs.mti_pitch =-MTI_ins.MTI_attitude.y;
	        ahrs.mti_yaw = MTI_ins.MTI_attitude.z;
			ahrs.set_mti_dcm();
	        ahrs.update_mti_atti_values();
			//mti_serial->printf(PSTR("roll=%f  pitch=%f yaw=%f\n"),MTI_ins.MTI_attitude.x,MTI_ins.MTI_attitude.y,MTI_ins.MTI_attitude.z);
		break;
		...
		case ACCEL:
			...
			ins.mti_accel = MTI_ins.MTI_acce;
			ahrs.set_mti_accel_ef();
			//mti_serial->printf(PSTR("accel.x=%f  accel.y=%f accel.z=%f\n"),MTI_ins.MTI_acce.x,MTI_ins.MTI_acce.y,MTI_ins.MTI_acce.z);
		break;
        ...
        case TURN_RATE:
			...
			MTI_ins.MTI_Gyr.x=fchar[m++]*DEG_TO_RAD_MTI;
		    MTI_ins.MTI_Gyr.y=-fchar[m++]*DEG_TO_RAD_MTI;
		    MTI_ins.MTI_Gyr.z=-fchar[m++]*DEG_TO_RAD_MTI;
			
			ins.mti_gyro  = MTI_ins.MTI_Gyr;
			//mti_serial->printf(PSTR("turn_rate.x=%f  turn_rate.y=%f turn_rate.z=%f\n"),MTI_ins.MTI_Gyr.x,MTI_ins.MTI_Gyr.y,MTI_ins.MTI_Gyr.z);
			r_state=0;
		break;
		...
		case AIRPRESSURE:
		    ...
		    MTI_ins.MTI_pressure=MTI_p.MTI_c;
			//barometer.set_mti_pressure(MTI_ins.MTI_pressure);
			//mti_serial->printf(PSTR("airpressure=%d\n"),MTI_ins.MTI_pressure);
		break;
		...
		case TEMPERATURE:
			...
			//barometer.set_mti_temp(MTI_ins.MTI_temp);
			//mti_serial->printf(PSTR("temperature=%f\n"),MTI_ins.MTI_temp);
		break;
		...
		case ALTITUDE:
			...
			ahrs.get_NavEKF().set_mti_altitude(MTI_ins.MTI_Alt);
			//barometer.set_mti_temp(MTI_ins.MTI_Alt);
			//mti_serial->printf(PSTR("altitude=%f\n"),MTI_ins.MTI_Alt);
		break;
		...
		case LNG_LAT:
		    ...
		    MTI_ins.MTI_Lat=fchar[m++]*10000000.0f;
		    MTI_ins.MTI_Lon=fchar[m++]*10000000.0f;
			//mti_serial->printf(PSTR("mti_longtitude=%d,mti_latitude=%d\n"),MTI_ins.MTI_Lon,MTI_ins.MTI_Lat);
		    gps.set_mti_location(MTI_ins.MTI_Lon,MTI_ins.MTI_Lat,MTI_ins.MTI_Alt);
		break;
...
int Copter::wrap_360_cd_yaw(int yaw_change)
{
    if (yaw_change > 360 || yaw_change < -360) {
        // for very large numbers use modulus
        yaw_change = yaw_change % 360;
    }
    while (yaw_change >= 360) yaw_change -= 360;
    while (yaw_change < 0) yaw_change += 360;
    return yaw_change;
}
