...
private:
	...
	AP_HAL::BetterStream* mti_serial;
...
//edit by huhui 
	struct  {
		Vector3f  MTI_acce;    //m/s^2
		Vector3f  MTI_Gyr;     //rad/s
		Vector3f  MTI_attitude;//rad
		Vector3f  MTI_Velocity;//m/s
		int32_t   MTI_Lat;
		int32_t   MTI_Lon;
		float     MTI_Alt;
		uint32_t  MTI_pressure;
		float     MTI_temp;
	}  MTI_ins;

	
    void read_mti_AHRS(void);
	void mtig_ReceiveDataByte(unsigned char recdata);
	void mtig_MessageReceived(int id, unsigned char* mtig_data, unsigned char len);
	bool mtig_data_success;
	unsigned char ch;
	unsigned char mtig_messageID;
	unsigned char buff[20];
	int mti_id;
    uint8_t r_state;
	uint8_t MTI_data_length;
	unsigned char sum;
	unsigned char MTI_Length ;
	unsigned char p_count;
    #define DEG_TO_RAD_MTI        0.0174532925f
	#define	ATTITUDE_ANGLE    0
	#define ACCEL             1
	#define	TURN_RATE         2
	#define VELOCITY          3
	#define AIRPRESSURE       4
	#define TEMPERATURE       5
	#define ALTITUDE          6
	#define LNG_LAT           7
	int  read_number;
	int wrap_360_cd_yaw(int error);
	bool tilt_control;
	void heli_update_tilt_servo_target();
	bool valid_ekf_origin;
	void set_ekf_origin();
